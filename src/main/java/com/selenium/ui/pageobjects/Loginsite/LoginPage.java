...
public class LoginPage extends BasePage {
    ...
    public LoginPage launchApplicationWithDoctors() {
        Reporter.log("********* " + new Throwable().getStackTrace()[0].getMethodName() + " *********");
        Objects.requireNonNull(driver, "Driver object is null...");
        setEnvironmentUrl();
        driver.get(GlobalPropertiesReader.propReader().Doctors_url());
        Reporter.log("Application is Launched with URL " + GlobalPropertiesReader.propReader().url());
        driver.manage().window().maximize();
        return this;
    }
    public LoginPage launchApplicationWithAgent() {
        Reporter.log("********* " + new Throwable().getStackTrace()[0].getMethodName() + " *********");
        Objects.requireNonNull(driver, "Driver object is null...");
        setEnvironmentUrl();
        driver.get(GlobalPropertiesReader.propReader().Agent_url());
        Reporter.log("Application is Launched with URL " + GlobalPropertiesReader.propReader().Agent_url());
        driver.manage().window().maximize();
        return this;
    }
    ...
    public void Agent_login() throws InterruptedException {
        Reporter.log("********* " + new Throwable().getStackTrace()[0].getMethodName() + " *********");
        enterValue(USERNAME, GlobalPropertiesReader.propReader().Agent_User(), "Username Field");
        enterValue(PASSWORD, GlobalPropertiesReader.propReader().Agent_password(), "Password Field");
        clickLoginButton();

    }
    public void Admin_login() throws InterruptedException {
        Reporter.log("********* " + new Throwable().getStackTrace()[0].getMethodName() + " *********");
        enterValue(USERNAME, GlobalPropertiesReader.propReader().Admin_Username(), "Username Field");
        enterValue(PASSWORD, GlobalPropertiesReader.propReader().Admin_password(), "Password Field");
        clickLoginButton();
